...
%  r2 = the GLM-CFC measure for the model using AmpLO
%  r_CI = the 95% confidence intervals for r.
%  r2_CI = the 95% confidence intervals for r2.
%  nCtlPts = the number of control points used.
%
%  By default, this function plots the results.

function [r, r_CI,r2,r2_CI, nCtlPts] = GLM_PAC(Vlo, Vhi, nCtlPts, varargin)
  ...
  Y = amp'; 
  X1 = spline_phase0(phi',nCtlPts);
  X2 = [X1,ampLO']; % the model takes into account the low-frequency amplitude
  XC = ones(size(Y));  
  ...
  X2eval = [X0,ampLO(10)*ones(size(phi0))']; %evaluate on some fixed amplitude
  ...
  %and for r2
  M = 10000;
  bMC = b2*ones(1,M) + sqrtm(stats2.covb)*normrnd(0,1,nCtlPts+1,M);
